...
def plugin_notebook2script(
    ...
    if isinstance(python_object, List):
        script = "\n".join(
            [
                plugin_notebook2script(
                    p, filename, return_instead_of_write=True
                )
        ...
        functions = [
            f[1]
            for f in functions
            if not f[0].startswith("_") or f[0] == "__init__"
        ]
        script = (
            "from accelergy.plug_in_interface.estimator import "
            "Estimator, action2energy\n"
        )
...
def yaml_section(
    ...
    tutorial.
    """
    content = yaml.load(open(file), Loader=yaml.Loader)
    for key in keys:
    ...
    return "\t" + "\n\t".join(
        yaml.dump(content, Dumper=yaml.RoundTripDumper).splitlines()
    )
...
def get_log_lines(logfile_path: str, start_from_line_containing: str) -> str:
    ...
    start = [i for i, l in enumerate(lines) if start_from_line_containing in l]
